...
const comparisonColors = ["#1d44b0", "#3b82f6", "#7c5cf5", "#a855f7", "#5b5fdb", "#4472ca", "#9061c9"];
...
const copy = {
  ...
  ko: {
    ...
    sourceKicker: "데이터 출처",
    sourceTitle: "데이터가 얼마나 최신인지 직접 확인하세요",
    sourceDesc:
...
function Sparkline({ values, positive }: { values: number[]; positive: boolean }) {
  ...
  if (values.length < 2) return <span className="event-sparkline event-sparkline-empty" aria-hidden="true" />;
  const width = 60;
  const height = 26;
  ...
  const coords = values.map((value, index) => [
    Number((index * stepX).toFixed(1)),
    Number((height - ((value - min) / range) * height).toFixed(1)),
  ]);
  const points = coords.map(([x, y]) => `${x},${y}`).join(" ");
  const areaPath = `M${coords[0][0]},${height} L${points.split(" ").join(" L")} L${coords[coords.length - 1][0]},${height} Z`;
  const color = positive ? "#e0433f" : "#2454e0";
  return (
    <svg
...
export function SignalAtlasDashboard({
  ...
  const marketTimeline = useMemo(() => {
    ...
    const sortedPrices = [...prices.entries()]
      .sort(([left], [right]) => left.localeCompare(right))
      .slice(-(timelineSessions + 1));
    const rows = sortedPrices
      .map(([date, close], index) => {
        ...
        const prevClose = index > 0 ? sortedPrices[index - 1][1] : null;
        return {
          date,
          label: date.slice(5),
          close,
          change: prevClose !== null ? close - prevClose : 0,
  ...
  const markerColors: Record<MarketEvent["coverage"], string> = {
    Direct: "#2454e0",
    Policy: "#4d8fea",
    Proxy: "#7a5fc4",
  };
    ...
    return (
      ...
      >
        ...
        >
          ...
          <circle {...commonProps} cx={marker.cx} cy={marker.cy} r={4.5} fill={markerColors[mapping]} />
        </g>
      </a>
    );
  };
      ...
      <main className="app-main">
        ...
        {latestSignal && (
          <section className="latest-signal section-shell">
            <div>
              {latestSignal.reaction ? (
                <span className="pending-pill computed">
                  <MetricValue value={latestSignal.reaction.abnormalReturn1D} />
                  <span className="computed-asset">{latestSignal.reaction.asset}</span>
                </span>
              ) : (
              ...
              <div>
                <small>
                  {latestSignal.reaction
                    ? locale === "ko"
                      ? "최신 TRUMP 시그널 · 시장 반응 확인됨"
                      : "LATEST TRUMP SIGNAL · MARKET REACTION OBSERVED"
                    : locale === "ko"
            ...
            <p>
              {latestSignal.reaction
                ? locale === "ko"
                  ? `${latestSignal.reaction.asset} 1일 초과반응이 ${latestSignal.reaction.eventSession} 세션 종가 기준으로 계산됐습니다 (거래량 ${latestSignal.reaction.volumeMultiple.toFixed(2)}배). 규칙 기반 매핑이며 인과관계가 아닙니다.`
                  : `${latestSignal.reaction.asset}'s one-day excess reaction was computed from the ${latestSignal.reaction.eventSession} session close (volume ${latestSignal.reaction.volumeMultiple.toFixed(2)}×). This is a rule-based mapping, not a causal claim.`
                : locale === "ko"
        ...
        <section className="section-shell market-timeline-section" id="market-timeline">
          ...
          >
            <div className="timeline-price-pane">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={marketTimeline.rows} syncId="market-timeline" margin={{ top: 18, right: 18, left: 2, bottom: 0 }}>
                  <defs>
                    <linearGradient id="timelinePriceFill" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor="#2454e0" stopOpacity={0.38} />
                      <stop offset="55%" stopColor="#2454e0" stopOpacity={0.12} />
                      <stop offset="100%" stopColor="#2454e0" stopOpacity={0} />
                    </linearGradient>
                  </defs>
                  <CartesianGrid stroke="#eef1ed" strokeDasharray="3 6" vertical={false} />
                  <XAxis dataKey="label" hide />
                  <YAxis domain={["auto", "auto"]} tickFormatter={(value) => `$${Number(value).toFixed(0)}`} tick={{ fill: "#8a938c", fontSize: 10 }} axisLine={false} tickLine={false} width={48} />
                  <Tooltip
                    labelFormatter={(label) => `${timelineAsset} · ${label}`}
                    formatter={(value, name) => name === "close" ? [`$${Number(value).toFixed(2)}`, locale === "ko" ? "종가" : "Close"] : [`$${Number(value).toFixed(2)}`, String(name).replace("Value", "")]}
                    contentStyle={{ background: "#14224f", border: "none", borderRadius: 14, boxShadow: "0 14px 30px rgba(15, 25, 55, 0.32)", fontSize: 12, padding: "9px 14px" }}
                    labelStyle={{ color: "#a9c3f5", fontWeight: 650, marginBottom: 3, fontSize: 10 }}
                    itemStyle={{ color: "#ffffff", fontWeight: 700 }}
                    cursor={{ stroke: "#c3cbe0", strokeWidth: 1, strokeDasharray: "3 4" }}
                  />
                  <Area isAnimationActive={false} type="linear" dataKey="close" stroke="none" fill="url(#timelinePriceFill)" connectNulls />
                  <Line isAnimationActive={false} type="linear" dataKey="close" name="close" stroke="#1d44b0" strokeWidth={2.4} strokeLinecap="round" strokeLinejoin="round" dot={false} activeDot={{ r: 6, fill: "#1d44b0", stroke: "#fff", strokeWidth: 3 }} connectNulls />
                  <Scatter dataKey="DirectValue" name="Direct" fill="#2454e0" tooltipType="none" shape={(props: unknown) => renderTimelineMarker(props, "Direct")} />
                  <Scatter dataKey="PolicyValue" name="Policy" fill="#4d8fea" tooltipType="none" shape={(props: unknown) => renderTimelineMarker(props, "Policy")} />
                  <Scatter dataKey="ProxyValue" name="Proxy" fill="#7a5fc4" tooltipType="none" shape={(props: unknown) => renderTimelineMarker(props, "Proxy")} />
                </ComposedChart>
              </ResponsiveContainer>
            </div>
            <div className="timeline-change-pane">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={marketTimeline.rows} syncId="market-timeline" margin={{ top: 4, right: 18, left: 2, bottom: 4 }}>
                  <XAxis dataKey="label" minTickGap={32} tick={{ fill: "#8a938c", fontSize: 10 }} axisLine={false} tickLine={false} />
                  <YAxis hide domain={["auto", "auto"]} />
                  <Tooltip
                    labelFormatter={(label) => `${timelineAsset} · ${label}`}
                    formatter={(value) => [`${Number(value) >= 0 ? "+" : ""}$${Number(value).toFixed(2)}`, locale === "ko" ? "전일 대비" : "Day change"]}
                    contentStyle={{ border: "1px solid #dbe1ee", borderRadius: 10, boxShadow: "0 12px 28px rgba(20, 34, 79, 0.14)", fontSize: 12, padding: "8px 12px" }}
                    labelStyle={{ color: "#33415f", fontWeight: 650, marginBottom: 2 }}
                    cursor={{ fill: "rgba(36, 84, 224, 0.05)" }}
                  />
                  <Bar dataKey="change" radius={[2, 2, 2, 2]} maxBarSize={10} isAnimationActive={false}>
                    {marketTimeline.rows.map((row) => (
                      <Cell key={row.date} fill={row.change >= 0 ? "#e0433f" : "#2454e0"} fillOpacity={0.85} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
        ...
        <section className="section-shell explorer-section" id="explorer">
          ...
          <div className="explorer-grid atlas-grid">
              ...
              {filtered.map((event) => (
                ...
                >
                  ...
                  <span className="row-reaction">
                    <Sparkline
                      values={(event.priceWindow ?? [])
                        .map((point) => point.close)
                        .filter((value): value is number => value !== null)}
                      positive={event.metrics.abnormalReturn1D >= 0}
                    />
                    <span className="row-reaction-value">
            ...
            <article className="evidence-panel">
              ...
              <div className="evidence-chart-col">
              <div
              ...
              <div className="chart-asset-switch">
                ...
                {lens === "market" && marketChartMode === "actual" && (
              ...
              <div className="reaction-chart actual-price-chart">
                <ResponsiveContainer width="100%" height="100%">
                  {marketChartMode === "compare" ? (
                    <ComposedChart data={comparisonData} margin={{ top: 18, right: 14, left: -5, bottom: 0 }}>
                      <CartesianGrid stroke="#eef1ed" strokeDasharray="3 6" vertical={false} />
                      <XAxis dataKey="label" tick={{ fill: "#8a938c", fontSize: 9 }} axisLine={false} tickLine={false} interval={1} />
                      <YAxis tickFormatter={(value) => `${Number(value).toFixed(1)}%`} tick={{ fill: "#8a938c", fontSize: 10 }} axisLine={false} tickLine={false} />
                      <Tooltip
                      ...
                      <ReferenceLine x={c.event} stroke="#7a5fc4" strokeDasharray="4 4" label={{ value: timing.split(" · ")[0], position: "insideTopRight", fill: "#7a5fc4", fontSize: 8 }} />
                      {chartAssets.map((symbol, index) => {
                        ...
                        return (
                          <Line
                            ...
                            name={symbol}
                            stroke={color}
                            strokeWidth={symbol === selected.asset ? 2.8 : 1.8}
                            strokeLinecap="round"
                            dot={{ r: symbol === selected.asset ? 3 : 2, fill: color, stroke: "#fff", strokeWidth: 1.4 }}
                    ...
                    >
                      ...
                      <ReferenceLine
                        ...
                        stroke="#7a5fc4"
                        strokeDasharray="4 4"
                        label={{ value: timing.split(" · ")[0], position: "insideTopRight", fill: "#7a5fc4", fontSize: 8 }}
                      />
                      <Line
                        isAnimationActive={false}
                        type="linear"
                        ...
                        strokeWidth={2.7}
                        strokeLinecap="round"
                        dot={{ r: 3, fill: "#1d44b0", stroke: "#fff", strokeWidth: 1.4 }}
                        activeDot={{ r: 6, fill: "#1d44b0", stroke: "#fff", strokeWidth: 2 }}
                        connectNulls
                      />
                    </LineChart>
                  ) : (
                    ...
                    >
                      ...
                      {(lens === "all" || lens === "attention") && (
                        <Line
                          ...
                          stroke="#7c5cf5"
                          strokeWidth={2}
                          dot={{ r: 2, fill: "#7c5cf5", stroke: "#fff", strokeWidth: 1 }}
                          connectNulls
                        />
                      )}
                      {lens === "attention" && (
              ...
              <div className="evidence-text-col">
