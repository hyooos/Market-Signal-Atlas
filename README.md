...
- Production: https://market-mover.vercel.app/
- Korean UI: https://market-mover.vercel.app/ko
...
* 게시 또는 보도 시각
* 인물, 주제, 연결 자산
* exact/date-only 시간 정밀도

### 2️⃣ Market Session Alignment
...
### 3️⃣ Multi-Source Reaction Check
...
A["공개 시그널 수집<br/>Public Signals"]
B["인물·주제·자산 매핑<br/>Entity · Topic · Asset Mapping"]
C["거래 세션 정렬<br/>Market Session Alignment"]

A --> B
B --> C

C --> D["가격·거래량 계산<br/>Return · Volume Metrics"]
C --> E["뉴스·공개 소셜 조회<br/>News · Social Search"]

D --> F["근거 검토<br/>Evidence Review"]
E --> F

F --> G["대시보드 탐색<br/>Timeline · Explorer · Universe"]
G --> H["한국어·영문 리포트<br/>Korean · English Report"]
```
...
| 뉴스 | Google News RSS, GDELT 보조, 검토 스냅샷 fallback |
...
│   ├── api/                     # live, signals, news, research, cron APIs
│   ├── ko/                      # Korean page route
│   ├── layout.tsx
│   └── page.tsx
...
├── data/                        # Source snapshots and generated JSON
│   ├── generated/
│   └── *.source.json

├── lib/                         # Data loading, metrics, orchestration
│   ├── metrics.ts
│   ├── orchestration.ts
│   ├── gdelt.ts
│   └── *.test.ts

├── scripts/                     # Dataset build and AI batch scripts
│   ├── build-events.mjs
...
# Optional server-only AI report helper
OPENAI_API_KEY=
...
- 무료 데모에는 X 전체 아카이브나 실시간 firehose가 없음
