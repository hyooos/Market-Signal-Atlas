# Market Signal Atlas — 첫 방문자 UI/UX 점검
...
1. 첫 행동 버튼가 `전체 데이터 보기`여서 핵심 분석이 아닌 대규모 카탈로그로 이동한다.
...
- 자산·기간·비교군을 차트보다 먼저 고르고, 주 차트에서 자산/비교 기준/동종 자산의 누적 초과수익률을 직접 비교한다.
...
| 차트·비교 | 실제로 제공되는 자산·기간·비교군·가격 창만 표시 | 아직 계산/데이터가 없는 동종 자산 비교, 실시간 분봉, 사건 마커를 완료 기능처럼 표시 |
...
| 데이터 상태 | 실제 공급자, 마지막 갱신 시각, `최신/오래됨/대기 중`과 결측 상태를 그대로 표시 | 최신 가격·뉴스·소셜 범위를 추정하거나 오래된 데이터를 실시간처럼 표시 |
| 미래 기능 | 구현 전에는 문서/로드맵에서만 `계획됨`으로 표시 | 홈 행동 버튼, 가격표, 메뉴에 즉시 이용 가능한 기능처럼 배치 |
...
4. 미래 기능은 행동 버튼가 아니라 `계획됨` 라벨이 붙은 로드맵에서만 노출한다.
...
- 보조 문구: 실제 가격·뉴스·대중 관심·AI 활용 준비 증거 감사
- 주요 행동 버튼: `전체 데이터 보기`
- 두 번째 행동 버튼: `분석 방식 보기`
...
- 두 행동 버튼 모두 제품의 핵심 작업(반응 비교·사건 분석)으로 바로 연결되지 않는다.
- 우측 레이더 장식과 `AI 활용 준비` 표현이 데이터 분석보다 AI 데모 같은 첫인상을 만든다.
- 한국어 화면인데 최신 신호 카드의 원문·`대기 중`·주제는 영어다. 원문을 보여 주는 것은 맞지만 “번역되지 않은 원문”이라는 표지와 짧은 한국어 요약이 없어 혼합 언어가 오류처럼 보인다.
...
- 주 행동 버튼은 `반응 비교 시작`으로 하고 `#comparison`의 새 시장 타임라인으로 연결한다.
- 보조 행동 버튼은 `대표 사건 보기`로 둔다. `전체 원문 검색`은 텍스트 링크 또는 하위 행동 버튼로 낮춘다.
...
### 1-2. 첫 행동 버튼: 핵심 도구보다 데이터 카탈로그를 먼저 보여 준다

![캡처 2 — `전체 데이터 보기` 클릭 직후](docs/ui-audit/02-universe-cta.png)

#### 사용자가 보게 되는 것

`전체 데이터 보기`를 누르면 32,393개 원문, 145,442개 원본 행, 군집 대표, 근거 보강, AI 활용 준비 파이프라인과 검색 테이블이 먼저 나온다. 그 뒤에야 사건 탐색 화면이 나온다.

#### 사용자 불편

- 처음 온 사용자는 “이 사이트에서 지금 무엇을 할 수 있는가?”보다 “데이터를 어떻게 쌓았는가?”를 먼저 학습해야 한다.
- 5단계 파이프라인, 5개 통계, 3개 범위 탭, 4개 필터가 한 번에 나온다. 탐색 목적이 분명하지 않은 상태에서 선택 부담이 크다.
- 기본 레이어가 `근거 준비 완료 735`인데, 상단에서 실제 값이 로드되기 전에는 0/—로 잠시 보인다. 첫 인상에서 데이터가 고장 난 것처럼 보일 수 있다.
- `AI 활용 준비가 된 조건부 오케스트레이션` 배지는 사용자가 결과의 신뢰도를 이해하는 데 직접 도움이 되지 않는다.

#### 개선안 — P0

- 시그널 유니버스를 메인 분석 흐름 **뒤** 또는 별도 `/universe` 페이지로 이동한다.
- 홈에서는 `전체 원문 32,393개 검색 가능`이라는 작은 신뢰 배지와 링크만 남긴다.
- URL/행동 버튼 이름도 목적에 맞춘다: `전체 원문 검색`, `데이터 범위 보기`. `전체 데이터 보기`라는 표현은 핵심 분석으로 이동할 것 같은 기대를 만든다.
- 데이터가 아직 로드되지 않은 경우 0이나 `—` 대신 카드형 로딩 뼈대 화면과 `목록 준비 중`을 보여 준다.
...
- 선택된 행은 배경색만이 아니라 `선택됨`, 자산, 날짜, 방향을 명확히 보여 준다.
- 중앙 상단에 `선택 사건: Elon Musk · TSLA · 2024-10-25` 같은 고정된 경로 표시를 표시한다.
...
- 선택 조건을 상단에 조건 칩으로 고정하고 `초기화`를 제공한다.
...
- `반응 비교`라는 서비스 핵심에 비해 실제로 비교되는 것은 선택 자산의 가격 창뿐이다. QQQ 비교 기준와 TSLA 동종 자산/다른 자산은 차트에 함께 나타나지 않는다.
...
- 차트를 원문 카드 안이 아니라 별도 주 영역으로 분리하고 데스크톱 기준 최소 `높이: 380px`을 확보한다.
- 기본 차트는 **누적 초과수익률 비교**로 바꾼다: `연결 자산 / 비교 기준 / 동종 비교군`. 가격(달러)은 `실제 가격 보기` 전환으로 분리한다.
...
- `근거 검토`는 차트 오른쪽의 얇은 요약 카드만 기본 노출한다: 판정, 신뢰도, 핵심 한계. 6단계는 `검토 과정 보기`를 눌러 확장한다.
...
- 기본 진입은 대표 시나리오 하나로 시작한다. 예: `Musk × TSLA / QQQ / EV 동종 자산`.
...
따라서 현재의 `시그널 탐색 → 반응 비교 → 데이터 출처 → 분석 방법` 순서는 바뀌어야 한다. 로고, 히어로 행동 버튼, 기본 앵커도 모두 `반응 비교`로 연결한다.
...
390px 폭에서 콘텐츠가 정상 폭으로 다시 배치되지 않고, 화면 왼쪽에 축소된 카드 일부만 보이며 오른쪽에 큰 빈 공간이 남는다. 첫 방문자가 제목·행동 버튼·탐색 기능을 읽고 누르기 어렵다.
...
| `AI 활용 준비`, `증거 리서치 데스크`, 6단계 상시 노출 | AI가 결론을 생산한다는 기대를 만듦 | `근거 검토`로 명명하고 단계는 접기. 규칙 기반·예측 아님을 한 번만 명확히 표시 |
| 7~10px 보조글자와 대문자 보조 라벨 다수 | 읽기보다 ‘대시보드 미학’에 치우친 인상 | 본문 14px, 보조 12px, 메타 11px 이상. 보조 라벨·배지는 꼭 필요한 상태에만 사용 |
...
2. `반응 비교`를 최상위 메뉴·히어로 행동 버튼·기본 진입으로 전환
3. `반응 비교`에 자산/기간/비교군/사건 마커가 있는 실제 타임라인 구현
4. 시그널 유니버스를 메인 분석 뒤 또는 별도 페이지로 이동
...
2. 0건 출처 탭 처리, 정렬 명칭 명확화, 선택 조건 조건 칩/초기화 제공
...
- 첫 행동 버튼가 전체 데이터셋이 아니라 실제 반응 비교 화면으로 이동한다.
- `반응 비교` 메뉴에서 적어도 자산·기간·비교군을 조작하고 사건을 클릭할 수 있다.
- 목록에서 보이는 선택 사건, 중앙 원문, 차트 제목, 우측 검토 요약이 항상 같은 사건을 가리킨다.
- 390px·768px·1440px에서 핵심 행동 버튼·필터·차트·선택 사건이 가로 잘림 없이 읽힌다.
